#!/usr/bin/env python3
# ============================================
# hassan_final_scalp_bot_v10.py
# V10 – Back to Solid Foundation + Real Improvements
# 6 Engines: OB, Whale, Momentum, Structure, Delta, RSI
# + Sharp Move + News + Min Liquidity + Wide SL
# ============================================
...
logger = logging.getLogger('HassanBotV10')
logger.handlers.clear()
console_handler = logging.StreamHandler()
console_handler.setFormatter(ColoredFormatter())
logger.addHandler(console_handler)
file_handler = RotatingFileHandler('hassan_bot_v10.log', maxBytes=10*1024*1024, backupCount=5)
file_handler.setFormatter(logging.Formatter('[%(asctime)s] %(levelname)s: %(message)s'))
logger.addHandler(file_handler)

# ============================================
# CONFIGURATION (CLEAN & OPTIMIZED)
# ============================================
CONFIG = {
    # === API & TELEGRAM ===
...
class UltimateSignalGenerator:
    ...
    async def generate(self, sym):
        ...
        # 🔧 Must have at least 2 vote difference
        if abs(lv-sv)<2: logger.info(f"⏭️ {sym} No consensus (L:{lv} S:{sv})"); return None
        ...
        # 🔧 Neutral trend: must have strong momentum
...
if TELE:
    class TelegramBot:
        ...
        async def startup(self):
            try: await self.bot.send_message(self.channel,"🚀 *ربات V10 فعال شد*\n✅ 6 Engine + Wide SL + Sharp Move",parse_mode=ParseMode.MARKDOWN)
            except: pass

        async def _start(self, u, c):
            kb=[["⚡ تحلیل اسکالپ","📊 سلامت"],["📋 معاملات","📈 گزارش"]]
            await u.message.reply_text("🤖 *V10*",reply_markup=ReplyKeyboardMarkup(kb,resize_keyboard=True),parse_mode=ParseMode.MARKDOWN)
...
class HassanBot:
    ...
    async def start(self):
        logger.info("🥇 V10 Starting – Solid Foundation")
